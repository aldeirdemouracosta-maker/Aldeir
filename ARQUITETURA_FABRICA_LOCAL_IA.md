...
- `orquestrador/`: loop de agente com tool calling (`ler_arquivo`,
  ...
  4 — ver `orquestrador/README.md` para o porquê dessa escolha. Com a
  flag `--diagnostico`, roda o `analisador_projeto` antes e passa o
  resultado como contexto inicial — o agente já começa sabendo TODOs,
  funções incompletas e testes falhando, em vez de descobrir por
  tentativa e erro.
- `analisador_projeto/`: diagnóstico "o que existe / o que falta" —
  linguagem principal, TODOs, funções incompletas em Python (via
  `ast`), módulos-esqueleto, e contagem de testes passando/falhando
  (rodados via `sandbox_execucao`, respeitando o portão do
